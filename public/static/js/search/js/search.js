		...
		initialize: function($searchBox, $searchResults) {
			...
			this.search = function(query, inApps, page, size) {
				if (query) {
					...
					timeoutID = window.setTimeout(function() {
						...
						$status.html('搜索其它位置'+'<img class="spinner" alt="search in progress" src="'+cloud_base.webroot+'/static/img/loading.gif" />');
			...
			function addResults(results) {
				...
				if (count === 0) {
					...
					$status.append('<h2>' + '在其他文件夹中没有搜索结果' + '</h2>');
				} else {
...
$(document).ready(function() {
	...
	if ($searchResults.length > 0) {
		...
		$searchResults.load(cloud_base.webroot + '/part.results.html', function () {
