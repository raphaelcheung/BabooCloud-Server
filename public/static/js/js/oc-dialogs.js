	...
	*/
	...
	prompt: function (text, title, callback, modal, name, password, buttonDeclineText, buttonConfirmText) {
		return $.when(this._getMessageTemplate()).then(function ($tmpl) {
			...
			$('body').append($dlg);
			var buttonlist = [{
					text : buttonDeclineText ? buttonDeclineText : '否',
					click: function () {
					...
					text         : buttonConfirmText ? buttonConfirmText : '是',
					click        : function () {
						if (callback !== undefined) {
	...
	*/
		...
		if (this._fileexistsshown) {
			...
			$.when(this._getFileExistsTemplate()).then(function($tmpl) {
				...
				var $dlg = $tmpl.octemplate({
					...
					allnewfiles: '新文件',
					allexistingfiles: '已经存在的文件',

					why: '你想保留哪些文件？',
					what: '如果选择两个版本，复制的文件将在其名称中添加一个数字。如果选择两个版本，复制的文件将在其名称中添加一个数字。'
				});
				$('body').append($dlg);
				...
				$(dialogId).on('click', '.replacement,.allnewfiles', function() {
					...
					if (count === $(dialogId+ ' .conflict').length) {
						...
						$(dialogId).find('.allnewfiles + .count').text('(全选)');
					} else if (count > 0) {
						$(dialogId).find('.allnewfiles').prop('checked', false);
						$(dialogId).find('.allnewfiles + .count').text('(选中 ' + count + ' 项)');
					} else {
						$(dialogId).find('.allnewfiles').prop('checked', false);
				...
				$(dialogId).on('click', '.original,.allexistingfiles', function(){
					...
					if (count === $(dialogId+ ' .conflict').length) {
						...
						$(dialogId).find('.allexistingfiles + .count').text('(全选)');
					} else if (count > 0) {
	...
	_getFilePickerTemplate: function() {
		...
		if(!this.$filePickerTemplate) {
			...
			//$.get(OC.filePath('core', 'templates', 'filepicker.html'), function(tmpl) {
				$.get(OC.filePath('filepicker.html'), function(tmpl) {
	...
	_getMessageTemplate: function() {
		...
		if(!this.$messageTemplate) {
			...
			$.get(OC.filePath('message.html'), function(tmpl) {
	...
	_getFileExistsTemplate: function () {
		...
		if (!this.$fileexistsTemplate) {
			...
			$.get(OC.filePath('fileexists.html'), function (tmpl) {
