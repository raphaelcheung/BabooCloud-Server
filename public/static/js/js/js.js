...
function fileDownloadPath(dir, file) {
	...
	return OC.filePath('download.php') + '?files=' + encodeURIComponent(file) + '&dir=' + encodeURIComponent(dir);
}
...
var OC = {
	 ...
	 * @see OC#getRootPath
	 */
	webroot: (typeof cloud_base !== 'undefined') ? cloud_base.webroot : '',
	...
	appswebroots: (typeof cloud_base !== 'undefined') ? cloud_base.webroot : false,
	/**
	 ...
	 */
	currentUser: (typeof oc_user !== 'undefined') ? cloud_base.uid : false,
	 ...
	 */
	linkTo: function (file) {
		//return OC.filePath(app, '', file);
		return OC.filePath(file);
	},

	linkToDirect: function (file) {
		link = OC.getRootPath();
		return OC.getRootPath() + '/' + file;
	},
...
/* 		if (oc_config.modRewriteWorking == true) {
	...
	fileIsBlacklisted: function(file) {
		return !!(file.match('\.(part|filepart)$'));
	},
	 ...
	 */
	addScript: function (script, ready) {
		//var deferred, path = OC.filePath(app, 'js', script + '.js');
		var deferred, path = OC.filePath(script + '.js');
	...
	addStyle: function (style) {
		...
		var path = OC.filePath(style + '.css');
	...
	appSettings: function (args) {
		...
		if (popup.is(':visible')) {
			...
			var jqxhr = $.get(OC.filePath(props.scriptName), function (data) {
				popup.html(data).ready(function () {
					popup.prepend('<span class="arrow ' + arrowclass + '"></span><h2>' + '设置' + '</h2><a class="close"></a>').show();
					...
					if (typeof props.loadJS !== 'undefined') {
						...
						$.getScript(OC.filePath(scriptname))
...
OC.Breadcrumb = {
	...
	_show: function (container, dir, leafname, leaflink) {
		...
		if (dir) {
			//add home
			var link = OC.linkTo('file/findfolder');
			...
			var crumbImg = $('<img/>');
			crumbImg.attr('src', OC.imagePath('static/img/places/home'));
			...
			jQuery.each(segments, function (i, name) {
				if (name !== '') {
					...
					var link = OC.linkTo('file/findfolder') + '?path=' + encodeURIComponent(pathurl);
