    ...
    FileList.prototype = {
        ...
        _onClickSelectAll: function(e, modifiedFiles) {
            ...
            if (checked) {
                for (var i = 0; i < filesViewed.length; i++) {
                    var fileData = filesViewed[i].name;
        ...
        _onClickDeleteSelected: function(event) {
            ...
            if (!this.isAllSelected()) {
                files = this.getSelectedFiles();//_.pluck(this.getSelectedFiles(), 'name');
            }
        ...
        _createRow: function(fileData, options) {
            ...
            if (this._allowSelection) {
                td.append(
                    ...
                    '<span class="hidden-visually">选择</span>' +
                    '</label>'
                );
            } else {
        ...
        changeDirectory: function(targetDir, changeUrl, force, fileId) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                self._uploads = {};
                self.reload().then(function(datas){
...
/*                     self._updateShareTree().then(function() {
        ...
        linkTo: function(dir) {
            return OC.linkTo('file/findfolder')+"?path="+ encodeURIComponent(dir).replace(/%2F/g, '/');
        },
        ...
        _setCurrentDir: function(targetDir, changeUrl, fileId) {
            ...
            if (!this._isValidPath(targetDir)) {
                OC.Notification.show('无效路径', {type: 'error'});
                targetDir = '/';
                changeUrl = true;
            }
            ...
            try{
                ...
                $.ajax({
                    ...
                    error: function(data, result){
                        console.log('file/findfolder failed...');
                        console.log(data);
                ...
                OC.Notification.show('无法拉取目录信息', {timeout: 7, type: 'error'});
                return;
            }
        ...
        reloadCallback: function(datas, result, response) {
            ...
            if (result != 'success'){
                ...
                OC.Notification.show('打开文件夹失败：' + datas.responseJSON
                    , {type: 'error'});
            ...
            // first entry is the root
             this.dirInfo = datas.shift();

            datas.sort(this._sortComparator);
            this.setFiles(datas);
        ...
        _setShareTreeUserGroupView: function() {
            ...
            _.each(self._shareTreeCache, function(folder) {
                ...
                _.each(shares, function(share) {
                    ...
                    var $path = $('<span>',   { class : 'shareTree-item-path', text : '通过' + " " + folder.name });
...
/*             this.$el.find('#permissions').val(permissions);
                ...
                self.filesClient.move(dir + fileName, targetPath + fileName)
                    ...
                    .fail(function(status, result) {
                        if (status === 412) {
                            ...
                            OC.Notification.show(fileName + '：无法移动文件，目标已存在', {type: 'error'}
                            );
                        } else if (status === 423) {
                            OC.Notification.show(fileName + '：无法移动文件，因为该文件或者目标文件已被锁定',
                                {type: 'error'}
                            );
                        } else if (status === 507) {
                            OC.Notification.show('可用空间不足',
                                {type: 'error'}
                            );
                        } else if (result != null && typeof result.message !== "undefined") {
                            OC.Notification.show(fileName + '：无法移动文件: ' + result.message,
                                {type: 'error'}
                            );
                        } else {
                            OC.Notification.show(fileName + '：无法移动文件',
                                {type: 'error'}
                            );
                        }
                    })
                        ...
                        self.filesClient.move(OC.joinPaths(path, oldName), OC.joinPaths(path, newName))
                            ...
                            .fail(function(status) {
                                ...
                                if (status === 404) {
                                    ...
                                    OC.Notification.show('无法重命名 "' + oldName + '", 文件不存在',
                                    ...
                                    OC.Notification.show(oldName + '：文件已被锁定，不能重命名',
                                        {type: 'error'}
                                    );
                                } else {
                                    // restore the item to its previous state
                                    OC.Notification.show(oldName + '：无法重命名',
                                        {type: 'error'}
                                    );
                                }
            ...
            self.filesClient.putFileContents(
                ...
                .done(function() {
                    ...
                    self.addAndFetchFileInfo(targetPath, '', {scrollTo: true}).then(function(status, data) {
                        ...
                        OC.Notification.show(name + '：无法创建文件',
                            {type: 'error'}
                        );
                    });
                })
                .fail(function(status) {
                    if (status === 412) {
                        OC.Notification.show(name + '：无法创建文件，因为文件已经存在',
                            {type: 'error'}
                        );
                    } else {
                        OC.Notification.show(name + '：无法创建文件',
        ...
        createDirectory: function(name) {
            ...
            try{
                $.ajax({
                    url: OC.filePath('file/createfolder') + '?path=' + encodeURIComponent(targetPath),
                    data: {},
                    type: "GET",
                    datatype: "json",
                    headers: {logintoken: cloud_token},
                    ...
                    success: function(data, result){
                        console.log('createDirectory suc!!');
                        console.log(data);
                        self.addDirInfo(data, {scrollTo:true});
                    ...
                    error: function(data, result){
                        ...
                        OC.Notification.show(targetPath + '：' + data.responseJSON, {type: 'error'});
                        deferred.reject(data, result);
         ...
         * @since 9.0
         */
        addDirInfo: function(dir, options){
            ...
            this.add(dir, addOptions);
         },
        addAndFetchDirInfo: function(path, options){
            var self = this;
            var deferred = $.Deferred();
            var promise = deferred.promise();

            if ((OC.dirname(path) || '/') !== this.getCurrentDirectory()) {
            ...
            $.ajax({
                url: OC.filePath('file/getfolder') + '?path=' + encodeURIComponent(path),
                data: {},
                type: "GET",
                datatype: "json",
                headers: {logintoken: cloud_token},
                ...
                cache: false,
                success: function(data, result){
                    console.log('file/getfolder suc!!');
                    console.log(data);
                ...
                error: function(data, result){
                    console.log('file/getfolder failed...');
                    console.log(data);

                    OC.Notification.show(path + '：无法创建目录', {type: 'error'});
                    deferred.reject(data, result);
                },
            });
...
/*             if (!_.isArray(files) && !files.is) {
            ...
            _.each(files, function(file) {
                ...
                if (state) {
                    ...
                    $thumbEl.css('background-image', 'url('+ OC.imagePath('static/img/loading.gif') + ')');
                } else {
...
/*             if (files && files.substr) {
            ...
            if (!files) {
                ...
                files = this.files;//_.pluck(this.files, 'name');
            }
            console.log(files);
            ...
            function removeFromList(filename) {
                var fileEl = self.remove(filename, {updateSummary: false});
            ...
            _.each(files, function(file) {
                if (file.isdir == true){
                    $url = OC.filePath('file/deletefolder') + '?path=' + encodeURIComponent(dir + '/' + file.name);
                }else{
                    $url = OC.filePath('file/deletefile') + '?filename=' + encodeURIComponent(dir + '/' + file.name);
                }
                ...
                $.ajax({
                    ...
                    error: function(datas, status){
                        if (status == 404){
                            ...
                            OC.Notification.show(file.name + '：无法删除文件，' + datas.responseJSON, {type: 'error'});

                            var deleteAction = self.findFileEl(file.name).find('.action.delete');
        ...
        updateEmptyContent: function() {
            ...
            this.$el.find('#emptycontent .uploadmessage').toggleClass('hidden', !this.isEmpty);
            this.$el.find('#emptycontent .nouploadmessage').toggleClass('hidden', true);
            ...
            if (this._filter && this.fileSummary.summary.totalDirs + this.fileSummary.summary.totalFiles === 0) {
                ...
                if ( $('#searchresults').length === 0 || $('#searchresults').hasClass('hidden') ) {
                    this.$el.find('.nofilterresults').removeClass('hidden').
                        find('p').text('此文件夹中没有匹配的文件 ' + this._filter);
                }
            } else {
...
/*             if (OCA.Search.files) {
        ...
        updateSelectionSummary: function() {
            ...
            if (summary.totalFiles === 0 && summary.totalDirs === 0) {
                this.$el.find('#headerName a.name>span:first').text('名称');
                this.$el.find('#headerSize a>span:first').text('大小');
                this.$el.find('#modified a>span:first').text('修改日期');
                ...
                var directoryInfo = summary.totalDirs +' 个文件夹';
                var fileInfo = summary.totalFiles + ' 个文件';
                ...
                if (summary.totalDirs > 0 && summary.totalFiles > 0) {
                    ...
                    selection = selectionVars.directoryInfo + ' 与 ' + selectionVars.fileInfo;
                ...
                if (!showHidden && summary.totalHidden > 0) {
                    var hiddenInfo = '包含 ' + summary.totalHidden + ' 个隐藏项目';
        ...
        _showPermissionDeniedNotification: function() {
            const message = '你没有权限在该目录下上传和创建文件';
        ...
        setupUploadEvents: function(uploader) {
            ...
            /*uploader.on('add', function(e, data) {
                ...
                if (data.context && data.context.data('type') === 'dir') {
                    ...
                    var translatedText = n('files', '正在上传 %n 个文件', '正在上传 %n 个文件', currentUploads);
                    if (currentUploads === 1) {
