...
(function() {
	var Files = {
		...
		_updateStorageStatistics: function(currentDir) {
			...
			state.call = $.getJSON(OC.filePath('getstoragestats.php') + '?dir=' + encodeURIComponent(currentDir),function(response) {
				state.dir = null;
				state.call = null;
				Files.updateMaxUploadFilesize(response);
			}).fail(function() {
				OC.Notification.show(
					'访问文件夹时遇到问题 ' + currentDir,
					{type : 'error'} 
				);
			});
		},
		...
		displayStorageWarnings: function() {
			...
			if (usedSpacePercent > 98) {
				if (owner !== currentUser) {
					OC.Notification.show(ownerDisplayName + ' 的存储空间已满，无法上传和更新文件！', 
						{type: 'error'}
					);
					return;
				}
				OC.Notification.show('你的存储空间已满，无法上传和更新文件！', 
					{type : 'error'}
				);
				return;
			}
			if (usedSpacePercent > 90) {
				if (owner !== currentUser) {
					OC.Notification.show(ownerDisplayName + ' 的存储空间快满了 (' + usedSpacePercent + '%)', 
						{  
							type: 'error'
						}
					);
				...
				OC.Notification.show('你的存储空间快满了 (' + usedSpacePercent + '%)',
					{type : 'error'}
				);
			}
		},
		...
		getAjaxUrl: function(action, params) {
			...
			return OC.filePath(action + '.php') + q;
		},
	...
	$(selectedFiles).each(function(i,elem) {
		...
		if (elem.type === 'dir') {
			newtr.find('td.filename')
				.css('background-image', 'url(' + OC.imagePath('static/img/filetypes/folder.png') + ')');
		} else {
