		...
		_renderInlineAction: function(actionSpec, isDefault, context) {
			...
			$actionEl.on(
				...
				function(event) {
					...
					if (!context.mime && context.fileList) {
						callContext.mime = $file.attr('data-mime');
					}
			...
			var defaultActions = [
				...
				},
				{
					...
					actionHandler: function (fileName, context) {
						...
						var file = context.fileList.getFile(fileName);

						context.fileList.do_delete([file], context.dir);
						$('.tipsy').remove();
					}
				}
			];
