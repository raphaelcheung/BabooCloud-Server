	...
	Files.prototype = {
		...
		initialize: function() {
			...
			this.renderFolderResult = function($row, result) {
				...
				$row.find('td.icon').css('background-image', 'url(' + OC.imagePath('static/img/filetypes/folder') + ')');
				return $row;
			};
			...
			this.renderFileResult = function($row, result) {
				...
				if (self.fileAppLoaded()) {
					...
					$row.find('td.icon').css('background-image', 'url(' + OC.imagePath('static/img/filetypes/' + mimeicon) + ')');
			...
			this.renderAudioResult = function($row, result) {
				...
				if ($row) {
					$row.find('td.icon').css('background-image', 'url(' + OC.imagePath('static/img/filetypes/audio') + ')');
				}
				return $row;
			};
			...
			this.renderImageResult = function($row, result) {
				...
				if ($row && !self.fileAppLoaded()) {
					$row.find('td.icon').css('background-image', 'url(' + OC.imagePath('static/img/filetypes/image') + ')');
				}
				return $row;
			};
